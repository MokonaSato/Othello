...
public class Board {
			...
			if (state == mine) {
				...
				for (int[] dir: direction) {
					...
					for (int j = 0; j < 8; j++) {
						...
						String neighboring_state = neighboring_stone.getState();
						//System.out.println("探索中(左)：(" + neighboring_pos[0] + ", " + neighboring_pos[1] + ")");
						
						if (neighboring_state == not_mine) {
							//System.out.println("自分のじゃない石");
							flag = true;
						}
						
						else if (neighboring_state == "・" && flag) {
							int[] neighboring_pos = {search_x, search_y};
							//System.out.println("左の石：(" + neighboring_pos[0] + ", " + neighboring_pos[1] + ")");
	...
	//インプットを受け付ける
	public int[] receiveInput(char x_input, char y_input) {
		...
		case 'H':
			...
			break;
		default:
			System.out.println("ERROR：x座標が存在しません。");
			System.out.println();
			x = 100;
		}
		int[] result = {x, y};
